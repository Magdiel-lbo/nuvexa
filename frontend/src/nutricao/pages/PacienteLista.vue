        ...
        <template #item.nome="{ item }">
          <div class="paciente-lista__nome-cell">
            <v-avatar color="primary" variant="tonal" size="31">
              <span class="paciente-lista__iniciais">{{ iniciais(item.nome) }}</span>
            </v-avatar>
            <div class="paciente-lista__nome-info">
              <span class="paciente-lista__nome-texto">{{ item.nome }}</span>
          ...
          <div class="d-flex justify-end ga-1">
            ...
            <v-menu v-if="podeExcluir">
              ...
              <v-list density="compact">
                <v-list-item prepend-icon="mdi-delete-outline" :title="$t('acao.excluir')" @click="excluir(item.id)" />
              </v-list>
            </v-menu>
          </div>
        </template>
...
import type { Objetivo, PacienteRelatorioFiltro, PacienteResponse, Sexo } from '../types/paciente'
...
interface PacienteLinha extends PacienteResponse {
...
export default class PacienteLista extends Vue {
  ...
  async mounted() {
    ...
    try {
      const [rotulos, enums] = await Promise.all([carregarRotulosEnum(), pacienteService.enums()])
      this.rotulos = rotulos
      this.opcoesSexoRaw = enums.sexos.map((opcao) => ({ value: opcao.valor, label: opcao.rotulo }))
      this.opcoesObjetivoRaw = enums.objetivos.map((opcao) => ({ value: opcao.valor, label: opcao.rotulo }))
      const [pacientes, consultas] = await Promise.all([pacienteService.listar(), consultaService.listar()])
  ...
  get headers(): NuvexaTableHeader[] {
    return [
      { title: this.$t('paciente.nome') as string, key: 'nome' },
  ...
  get filtrados(): PacienteResponse[] {
    const busca = normalizar(this.filtros.value('q'))
    const sexo = this.filtros.value('sexo')
    const objetivo = this.filtros.value('objetivo')
    const imcSelecionado = this.filtros.values('imc')

    const resultado = this.pacientes.filter((paciente) => {
      if (busca && !normalizar(paciente.nome).includes(busca)) return false
  ...
  get linhas(): PacienteLinha[] {
    return this.filtrados.map((paciente) => ({
      ...paciente,
      ultimaConsultaLabel: this.ultimasConsultas[paciente.id] ? this.formatarData(this.ultimasConsultas[paciente.id]) : null,
    }))
  }
  ...
  async exportar() {
    ...
    try {
      ...
      const blob = await pacienteService.relatorioExcel(filtro)
