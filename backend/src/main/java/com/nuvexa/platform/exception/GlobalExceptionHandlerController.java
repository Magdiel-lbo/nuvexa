...
public class GlobalExceptionHandlerController {
    ...
    public ResponseEntity<ApiErro> handleUnreadableBody(HttpServletRequest request) {
        return buildResponse(HttpStatus.BAD_REQUEST, mensagens.getMessage("erro.corpoInvalido"), request);
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public ResponseEntity<ApiErro> handleTypeMismatch(HttpServletRequest request) {
        return buildResponse(HttpStatus.BAD_REQUEST, mensagens.getMessage("erro.parametroInvalido"), request);
    }
    ...
    public ResponseEntity<ApiErro> handleAccessDenied(AccessDeniedException ex, HttpServletRequest request) {
        ...
        return buildResponse(HttpStatus.FORBIDDEN, mensagens.getMessage("erro.acessoNegado"), request);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiErro> handleUnexpectedError(Exception ex, HttpServletRequest request) {
        log.error("Erro inesperado", ex);
        return buildResponse(HttpStatus.INTERNAL_SERVER_ERROR, mensagens.getMessage("erro.inesperado"), request);
    }
