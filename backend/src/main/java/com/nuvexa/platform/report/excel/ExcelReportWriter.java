package com.nuvexa.platform.report.excel;
...
public final class ExcelReportWriter {
    ...
    public static <T> byte[] write(String sheetName, List<ReportColumn<T>> columns, List<T> rows) {
        List<ReportColumn<T>> sortedColumns = columns.stream()
    ...
    private static <T> void writeHeader(Workbook workbook, Sheet sheet, List<ReportColumn<T>> columns) {
    ...
    private static <T> void writeRows(Sheet sheet, List<ReportColumn<T>> columns, List<T> rows) {
        int rowIndex = 1;
        for (T row : rows) {
